...
import dev.codetrail.desktop.controller.SimulationInputFieldController;
import dev.codetrail.desktop.controller.SimulationPlayerWindow;
...
/**
 * Embedded VisuAlgo simulation controller in the lesson viewer.
 * Provides high-fidelity Canvas rendering, pseudocode tracing, step playback,
 * custom input editing, and full Studio popout.
 */
@Component
...
public final class SimulationController {
    ...
    private String currentRawType = "ARRAY";
    private String currentConfigJson = "{}";
    private String simulationType = "ARRAY";
    ...
    @FXML private Button openStudioButton;
    @FXML private Label statusLabel;

    public SimulationController() {
    }

    public SimulationController(ObjectMapper unused) {
    }
    ...
    private void initialize() {
        if (narrationLabel != null) {
            ...
            narrationLabel.widthProperty().addListener((obs, o, n) -> updateCaption());
            narrationLabel.heightProperty().addListener((obs, o, n) -> updateCaption());
            narrationLabel.fontProperty().addListener((obs, o, n) -> updateCaption());
        }
        if (playbackModeChoice != null) {
            ...
            playbackModeChoice.valueProperty().addListener((obs, prev, val) -> {
                stopPlayback();
                teachingMode = !"All steps".equals(val);
                if (cursor != null) renderCurrent();
            });
        }
        if (simulationSplit != null && !simulationSplit.getDividers().isEmpty()) {
            simulationSplit.getDividers().getFirst().positionProperty().addListener((obs, prev, val) -> {
                if (!adjustingSplit && "visualization".equals(currentView) && !focusDiagram && !projectorMode) {
                    splitPosition = val.doubleValue();
                }
            });
        }
        ...
        if (speedSlider != null) {
            speedSlider.valueProperty().addListener((obs, o, n) -> updatePlaybackRate());
            updatePlaybackRate();
        }
        if (canvasContainer != null) {
            canvasContainer.widthProperty().addListener((obs, o, n) -> resizeCanvas());
            canvasContainer.heightProperty().addListener((obs, o, n) -> resizeCanvas());
        ...
        if (pseudocodeScrollPane != null) {
            pseudocodeScrollPane.viewportBoundsProperty().addListener((obs, o, n) -> {
                if (cursor != null) {
        ...
        if (pseudocodeBox != null) {
            pseudocodeBox.heightProperty().addListener((obs, o, n) -> {
                if (cursor != null) {
                    ensureActiveLineVisible(cursor.current().highlightedPseudocodeLine());
                }
            });
    ...
    public void load(SimulationView simulation) {
        ...
        load(simulation.type(), simulation.configJson());
    }

    public void load(String rawType, String configJson) {
        stop();
        this.currentRawType = rawType;
        this.currentConfigJson = configJson == null ? "{}" : configJson;
    ...
    private void onOpenStudio() {
        stopPlayback();
        if (simulationRoot != null && simulationRoot.getScene() != null) {
            SimulationPlayerWindow.open(currentRawType, currentConfigJson, isDarkMode(), simulationRoot.getScene().getWindow());
        } else {
            SimulationPlayerWindow.open(currentRawType, currentConfigJson, isDarkMode(), null);
        }
    }
    ...
    private void generateTrace(JsonNode input, String source) {
        ...
        CompletableFuture.supplyAsync(() -> engine.generateTrace(requestedInput))
                .whenComplete((generated, failure) -> Platform.runLater(() -> {
                    ...
                    if (failure != null) {
                        ...
                        String rawErr = cause.getMessage() == null ? "Check your input and try again." : cause.getMessage();
                        String message = inputAdapter == null ? rawErr : inputAdapter.friendlyError(rawErr);
    ...
    private void renderPseudocode() {
        ...
        for (int index = 0; index < metadata.pseudocode().size(); index++) {
            ...
            codeLine.fontProperty().addListener((obs, oldFont, newFont) -> alignIndentation.run());
