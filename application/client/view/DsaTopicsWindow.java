...
public class DsaTopicsWindow {
    ...
    private static HBox buildTopToolbar(Stage stage, Window owner) {
        ...
        Button arenaHeaderBtn = new Button("Problem Arena (80)");
        ...
        arenaHeaderBtn.setOnAction(e -> {
            ...
            DsaProblemArenaWindow.show(stage, isDark, "data structures", null, ProgrammingLanguage.CPP);
        });
        ...
        topBar.getChildren().addAll(brandMark, brandTitleBox, badge, arenaHeaderBtn, spacer, backBtn);
        return topBar;
    }
    ...
    private static VBox buildSidebar(Stage stage, Consumer<String> onSelectModule) {
        ...
        Button arenaSidebarBtn = new Button("Coding Exercises & Judge (80)");
        arenaSidebarBtn.getStyleClass().add("dsa-sidebar-arena-btn");
    ...
    private static VBox buildTopicCard(DsaTopic topic, Stage stage, Consumer<String> onSelectModule) {
        ...
        boolean isDs = "data structures".equalsIgnoreCase(topic.moduleKey());
        ...
        exercisesButton.setOnAction(event -> {
            event.consume();
            boolean isDark = stage.getScene() != null && stage.getScene().getRoot().getStyleClass().contains("dark-theme");
            DsaProblemArenaWindow.show(stage, isDark, isDs ? "arrays" : topic.moduleKey(), null, ProgrammingLanguage.CPP);
        });
        ...
        if (isDs) {
            MenuItem allDsItem = new MenuItem("⚡ Open All Data Structure Exercises (45 Problems)");
            allDsItem.setOnAction(e -> {
                boolean isDark = stage.getScene() != null && stage.getScene().getRoot().getStyleClass().contains("dark-theme");
                DsaProblemArenaWindow.show(stage, isDark, "arrays", null, ProgrammingLanguage.CPP);
            });
            contextMenu.getItems().add(allDsItem);

            Menu dsSubmenu = new Menu("📚 Select Sub-Topic (5 Problems Each)");
            String[][] dsTopics = {
                    {"Arrays", "arrays"},
                    ...
                    {"Trie (Prefix Trees)", "trie"}
            };
            for (String[] sub : dsTopics) {
                MenuItem subItem = new MenuItem(sub[0] + " (5 Problems)");
                final String subKey = sub[1];
                subItem.setOnAction(e -> {
                    boolean isDark = stage.getScene() != null && stage.getScene().getRoot().getStyleClass().contains("dark-theme");
                    DsaProblemArenaWindow.show(stage, isDark, subKey, null, ProgrammingLanguage.CPP);
                });
                dsSubmenu.getItems().add(subItem);
            }
            contextMenu.getItems().add(dsSubmenu);
        } else {
            MenuItem exerciseItem = new MenuItem("Open Exercises & Judge (5 Problems)");
            exerciseItem.setOnAction(e -> {
                boolean isDark = stage.getScene() != null && stage.getScene().getRoot().getStyleClass().contains("dark-theme");
                DsaProblemArenaWindow.show(stage, isDark, topic.moduleKey(), null, ProgrammingLanguage.CPP);
            });
            contextMenu.getItems().add(exerciseItem);
        }
    ...
    private static HBox buildFooterBar(Stage stage, Consumer<String> onSelectModule) {
        ...
        Button openArenaFooterBtn = new Button("Problem Arena (80 Problems)");
        openArenaFooterBtn.getStyleClass().add("dsa-footer-arena-btn");
        openArenaFooterBtn.setOnAction(e -> {
            boolean isDark = stage.getScene() != null && stage.getScene().getRoot().getStyleClass().contains("dark-theme");
            DsaProblemArenaWindow.show(stage, isDark, "data structures", null, ProgrammingLanguage.CPP);
        });
        ...
        footer.getChildren().addAll(spacer, openArenaFooterBtn, startFirstBtn);
        return footer;
    }
}
