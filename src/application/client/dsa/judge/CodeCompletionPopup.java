...
public class CodeCompletionPopup {
    ...
    private final PauseTransition suggestionDebounce = new PauseTransition(Duration.millis(90));
    ...
    public CodeCompletionPopup(EditorAdapter editor, boolean isDark) {
        ...
        this.suggestionDebounce.setOnFinished(e -> showSuggestionsForCurrentCaret());
    ...
    private void installEditorKeyFilters() {
        ...
        // Typing triggers suggestions and auto-closing pairs (debounced to keep typing silky smooth)
        editor.addEventFilter(KeyEvent.KEY_TYPED, event -> {
            ...
            if (handleAutoClosingPairs(ch)) {
                event.consume();
                suggestionDebounce.playFromStart();
                return;
            }

            // Trigger suggestion check on letters/digits with debounce
            char c = ch.charAt(0);
            if (Character.isLetterOrDigit(c) || c == '_' || c == '#' || c == ':' || c == '.') {
                suggestionDebounce.playFromStart();
            } else {
